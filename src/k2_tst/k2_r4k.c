...
char test_K2[40][15]={{0x55, 0x04, 0x01, 0x0B, 0x65},		// ->+ 0
					...
					//{0x55, 0x05, 0x01, 0x2C, 0x00, 0x8B}, //29 R4K 0
					//{0x55, 0x05, 0x01, 0x2C, 0x02, 0x89}, //29 R4K -
					//{0x55, 0x05, 0x01, 0x05, 0x02, 0x62}, //30 S4K -
					{0x55, 0x05, 0x01, 0x05, 0x04, 0x64}, //30 S4K 0
...
/*write_com24 (Ncom) //7
{
	...
	short V=0;
	char A=0;
	V=(short)(p->from41.Vr);	if (TS) V=1;
	test_K2[24][4]=V&0x00ff; 	test_K2[24][5]=V>>8;
	printf("V=%d",V);
	A=(char)(p->from41.Ar); 	if (TS) A=1;
	test_K2[24][6]=A;
	printf(" dV=%d\n",V);
...
write_com24 (Ncom) //8
{
	int i1, i;
	short V=0;
	V=(short)(p->from41.Vr); 	if (TS) V=0;
	test_K2[24][4]=V&0x00ff; 	test_K2[24][5]=V>>8;
	printf("V=%d",V);
	//V=(short)(p->from41.Ar/1.5625);
	V=(short)(p->from41.Ar);	if (TS) V=0;
...
/**/			case 9: if (comOK[5]>0) {comOK[5]=Ncount=0;N_COM+=4;break;}
			   ...
			   case 22: read_kvit();break;
 			   case 23: write_com(29);printf(")  Команда РЧК 0 отправлена\n");break;
			   case 24: read_kvit();break;
 			   case 25: write_com(30);printf(")  Команда CЧК 0 отправлена\n");break;
			   case 26: read_kvit();break;
			   ...
			   case 29:	write_com(10);printf(")  Команда НУС отправлена\n");break;
			   case 30: read_kvit_NUS();break;
			   case 31: if (comOK[10]>0) {comOK[10]=Ncount=0;N_COM+=9;break;}//9
						if (Tcount>Tcount_com+500) {printf("Timeout 50 sec\n");N_COM=-2;}
						if (Ncount>10) {printf("TPO not responding\n");N_COM=0;}
			   ...
			   case 33: read_kvit();break;
 			   case 34: write_com(28);printf(")  Команда ЗДР.З отправлена\n");break;
			   case 35: read_kvit();break;
 			   case 36: write_com(29);printf(")  Команда РЧК отправлена\n");break;
			   case 37: read_kvit();break;
 			   case 38: write_com(30);printf(")  Команда СЧК отправлена\n");break;
			   case 39: read_kvit();break;
 			   case 40: write_com24(24);printf("  Начата посылка команды ДанИ.V.dV\n");
						comOK[24]=1;N_COM++;
...
//----------------------------------------------------------------------------
			   case 41: if (TM) {N_COM++;break;}
						...
						break;
			//-----------------------------------
 			   case 42: write_com27(27);printf(")  Команда ДанП.Т.Р отправлена\n");
						N_COM++;				
						break;
			   case 43: read_kvit();break;
			//-----------------------------------
 			   case 44: write_com(31);printf(")  			Команда Сеанс.L отправлена\n\n");
						break;
			   case 45: read_kvit();break;
 			   case 46: if (TS) {if (Tcount>Tcount_com+40) {write_com(32);printf("\n\nОБР ТЕСТ\n\n");}}
						else N_COM++;
						break;			   
 			   case 47: if (TS)	
						{
							if (Tcount>Tcount_com+100) {write_com(23);N_COM=41;}
							else if (comOK[24]>0) {comOK[24]++;if (comOK[24]>10) {comOK[24]=1;write_com24(24);}} 
						}
						else 
						{
							if (Tcount>Tcount_com+240) {write_com(23);N_COM=41;}
							else if (Tcount>Tcount_com+10) {comOK[24]++;if (comOK[24]>10) {comOK[24]=1;write_com24(24);}} 
						}
						break;
			   case 48: read_kvit();break;
			}
